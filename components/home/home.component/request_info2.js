...
export const RequestInfo2 = ({item, navigation, code}) => {
  ...
  useEffect(() => {
    if (item.unread) {
      setTimeout(() => {
        database()
          .ref(`/requests/${profile.uid}/${item.id}`)
          .update({unread: false})
          .then(() => {
    ...
    database()
      ...
      .then(() => {
        ...
        if (item.status != 1) {
          ...
          sendPushNotification(
            'Request Cancelled',
            `Request is cancelled by ${profile.name}`,
            0,
            tokens,
            navigate,
          );
      ...
      <View
        ...
        {isOnline ? (
          ...
          <Image
            ...
            source={
              item.twoWay
                ? require('../../assets/home_main/home/twoArrow.png')
                : require('../../assets/home_main/home/oneArrow.png')
            }
          />
        )}
          ...
          {item.status == 2 && isOnline ? (
            <>
              <Text
                ...
                {item.accepted
                  ? `${item.accepted} ${
                      item.accepted == 1 ? 'driver is' : 'drivers are'
          ...
          <>
            {(item.status == 1 || item.status == 2) && (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('RequestRide', {preReq: item})
                }>
                <Image
                  style={{
                    height: myHeight(2),
            ...
            {item.driverContact ? (
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    Linking.openURL(`tel:${item.driverContact}`);
                  }}>
                  <Image
