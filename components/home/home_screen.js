...
export const HomeScreen = ({navigation}) => {
  ...
  function updateOnlineDrivers() {
    ...
    dispatch(
      setOnlineDriver(
        driv.sort(function (a, b) {
          return a.distanceInt - b.distanceInt;
        }),
      ),
    );
  ...
  useEffect(() => {
              ...
              if (val.status == 2 || val.status == 3) {
                ...
                const onlineStatus =
                  item.status == 2
                    ? accepted
                      ? `${accepted} ${
                          accepted == 1 ? 'driver is' : 'drivers are'
                        } waiting for your response`
                      : 'No driver responded yet'
                    : 'In Progress';
                const onlineStatusColor =
                  item.status == 2 && !accepted ? myColors.red : myColors.green;
                ...
                const onlineStatusColor =
                  item.status < 0 ? myColors.red : myColors.green;
    ...
    // Stop listening for updates when no longer required
    return () =>
      database().ref(`/requests/${profile.uid}`).off('value', onValueChange);
  }, []);
  // Realtime
  useEffect(() => {
  ...
  useEffect(() => {
    if (profile.city) {
      ...
      getDashboardData(setIsLoading, profile);
    }
      ...
      <View key={(code = 0)}>
        <View
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.6}
            onPress={() => navigation.navigate('Search', {name, code})}>
            <Text
              style={[
                styles.textCommon,
                {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <View
          style={{
            ...
            borderColor: myColors.primaryL5,
          }}>
          <TouchableOpacity
            activeOpacity={0.75}
            style={{
      ...
      {progress.length ? <Status notifications={progress} /> : null}
