...
export const Login = ({
  ...
  function checkUser() {
    ...
    FirebaseUser.where('email', '==', email)
      .get()
      .then(result => {
        if (result.empty) {
          showError('Incorrect email or password');
      ...
      <View>
        ...
        <View>
          <Text
            style={[
              styles.heading,
              {color: email ? myColors.textL4 : myColors.text},
            ]}>
            Email address
          </Text>
        ...
        <View>
          <Text
            style={[
              styles.heading,
              {color: password ? myColors.textL4 : myColors.text},
            ]}>
            Password
          </Text>
...
const styles2 = verifyLog =>
  StyleSheet.create({
    textReg: {
      color: verifyLog ? myColors.background : myColors.textL4,
      fontFamily: myFonts.headingBold,
      fontSize: myFontSize.body,
    },
  });
